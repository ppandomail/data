...
prod_cant = int(input('Cant. de productos: '))
prod_total = round(prod_precio * prod_cant, 2)

prod_precio = str(prod_precio).split('.')
prod_cant = str(prod_cant)
prod_total = str(prod_total).split('.')

print(prod_nombre + ' ' + 
      prod_precio[0].zfill(6) + '.' + prod_precio[1] + ' ' + 
      prod_cant.zfill(3) + ' ' + 
      prod_total[0].zfill(8) + '.' + prod_total[1])